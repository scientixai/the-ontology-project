...
**Agent (4)** — `Person`, `Organization`, `Group`, `AutonomousAgent`. Person → `prov:Person`; Organization → `prov:Organization`; Group → `prov:Agent`; AutonomousAgent → `prov:SoftwareAgent`. (Per ADR-0022, agency is a role, not a kind: a living non-human bears no accountability, so `top:Organism` is not an Agent leaf. Biological kinds specialize `top:CommonEntity` in their domain and bind into operations via `top:hasSubject`; the shared "Organism" vocabulary anchor is retained in `taxonomy/taxonomy.ttl`.)
