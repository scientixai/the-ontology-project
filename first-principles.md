...
* **NGSI-LD temporal properties:** attributes operators watch change (Status, Phase) are temporal streams — `observedAt` carries the valid-time observation instant, `validFrom` / `validUntil` the valid-time interval; the transaction clock is `recordedAt` (ADR-0021 erratum, 2026-09-01).
