...
**Study** lifted to v0.3.0-strawman as the third top-level (`study-spec.html`, 659 lines) — 12 verification questions pending Bo's review. Study is the central organizing entity: Sponsor.runs and StudySite.forStudy both anchor on it; Study itself owns the design-side chain (Protocol, Arms, Schedule of Assessments, Endpoints, Inclusion/Exclusion Criteria) as sub-objects. USDM v3's three-level Study → StudyVersion → StudyDesign nesting is flattened (with StudyVersion sub-object lift in v0.4 if richer multi-version semantics warrant). Outcomes:

- **Study top-level** lifted from 4 attrs / 2 rels to 36 attrs / 16 rels with full design-content coverage (identity, type/phase/design parameters, population/eligibility framing, lifecycle/dates, multi-registry IDs).
- **6 sub-objects** under Study: Protocol (10 attrs), Arm (9), ScheduleOfAssessments (8), Endpoint (8), InclusionCriterion (6), ExclusionCriterion (6).
- **6 new SHACL invariants** (16 total now: 4 Sponsor + 6 Site/StudySite/System/Credential + 6 Study). Lifecycle invariants encode: at-or-past-RECRUITING requires endpoints/inclusion/SOA; INTERVENTIONAL requires phase; multi-arm interventionModel requires multiple Arms; COMPLETED should have actualCompletionDate (soft warning).
- **All 4 worked examples** (sponsor-pfizer-iqvia, site-mskcc-onco423, site-elevate-network, site-iit-mskcc) extended with Endpoint, InclusionCriterion, and ScheduleOfAssessments entities; all conform under pyshacl 0.31.0 advanced mode.
