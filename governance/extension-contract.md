...
The three flavors govern how a consumer *extends* a Core concept. This rule governs the two prior questions: **(1) is this even a class, or a property? and (2) if a class, is it the consumer's to mint, or does a parent layer already own it?** It exists because an operator-elicited object catalog (OOUX) has no import statement — it reads its own completeness as self-containment, re-mints concepts Core or a mid-layer already owns, *and* reifies attributes, relations, and context into "objects." See [`docs/ooux-layer-blindness.md`](../docs/ooux-layer-blindness.md) for the failure that motivated it.

Reconciliation runs on two axes. **KIND** (Jessica Talisman: *"context is a property, not an object; classes are the TBox — small — and properties are the ABox, the data"*): is the term a class or a property/relation/context? **TIER** (only for classes): which layer owns it? Three violations, all mechanical, all blocking at PR time:

- **Reify.** A term recorded as a property / relation / context (`kind` ≠ `class`) but modeled as an `owl:Class` or a SHACL `sh:targetClass`. A date, a tag, a role, a context is not an object — demote it to a property.
- **Shadow.** A domain class (or `sh:targetClass`) whose **local name equals an existing parent-layer class** (`top:X` / `hcls:X`). Re-minting a parent concept is forbidden — dedupe to it or subclass it.
- **Orphan.** A domain class with **no `rdfs:subClassOf` chain to a parent layer** and no recorded tier decision. Every domain class must cite its parent world.

**Resolution is recorded, not implicit.** Each domain's object set carries a tier map (`<domain>/views/tier-map.json`) with a `kind` and a verdict per object — `demote` (not a class; re-model as a property), `dedupe` (bind to an existing parent), `subclass` (mint native, `subClassOf` a named parent), `promote` (a cross-domain primitive with no parent yet — a Core/mid-layer candidate, RFC-gated), or `review` (tier decision pending). `dedupe`/`subclass` pass; `demote`/`promote`/`review` are tracked backlog. An object **absent** from the map is untriaged and fails the gate — which is what stops recurrence: a newly introduced term must be classified on both axes before it can land.
