...
Per [ADR-0017](decision-log.md#adr-0017-monorepo-with-directory-scoped-ownership), TOP is a single repository with directory-scoped ownership. A working group's authority and accountability map to a directory tree.
...
| **Clinical Research WG** | `/clinical-research/` (directory created when rebuild starts) | Forming |
| **Care Delivery WG** | `/care-delivery/` (directory created when work starts) | Proposed |
| **Manufacturing WG** | `/manufacturing/` (directory created when work starts) | Proposed |
| **Supply Chain WG** | `/supply-chain/` (directory created when work starts) | Proposed |

The convener (currently Bo Lora) holds Core stewardship until a Core stewards committee forms. As workflow WGs activate, they take ownership of their directories per [`.github/CODEOWNERS`](../.github/CODEOWNERS).
...
- **Reintroduce bespoke boolean flags or undocumented enums.** Per [ADR-0015](decision-log.md#adr-0015-promote-facts-to-entities-no-bespoke-flags), facts that have a time, an authority, a scope, or evidence must reify to Core primitives — Attestation, Constraint, StatusChange — not collapse into booleans.
...
- The WG's directory is created (e.g., `/clinical-research/`) when the first artifact is ready to commit.
...
- `.github/CODEOWNERS` still resolves the placeholder line to the convener; the WG team handle is registered but not yet active.
