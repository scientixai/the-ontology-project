...
- **TMF onboarding & completeness** ✅ the day-one journey on top of the binding map: a customer's filing cabinet onboarded as typed, bitemporal, provenanced `tmf:FiledArtifact` instances, judged against a reusable essential-documents checklist (`tmf:CompletenessProfile` — the DIA/ICH list as data). Inspection-readiness is graded — a required type with no **current** filed artifact = Violation (a superseded/expired filing does not satisfy it), a recommended gap = Warning, an unclassified filing = Warning — plus a completeness projection (expected-vs-filed diff, GAP detection) and a **single-pull TMF dashboard** view that returns the expected set *and* the filed set in one pull, so the screen diffs them at the glass. Classifying real PDFs / extracting facts stays the consuming system's job.
...
Current suite: SHACL 59/59 · bitemporal 3/3 · projections 20/20 · demo 1/1 · safety 2/2 · pre-IND 2/2 · lims 1/1 · startup 2/2 · schedule 3/3 · usdm 1/1 · ncit 1/1 · view 14/14 · owl-lint 2/2.
