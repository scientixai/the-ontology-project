...
- **Single-pull retrieval view (NGSI-LD, no recursive lookups)** ✅ the test of the model: an operator pulls **one** entity (the blood-draw collection act) and gets *every* fact behind the green check — participant + current consent, the phlebotomist's venipuncture delegation + credential, the SoA ordering, and specimen → custody → aliquot → result — inlined into **one self-contained object**. Faithful to **ETSI GS CIM 009** Linked Entity Retrieval: `?join=inline&joinLevel=3` embeds each linked Entity as the `entity` sub-attribute of its Relationship (clause 4.5.23.2), following relationships **forward** only (clause 4.5.23.1). That forward-only rule drove a **modeling correction** — direct forward retrieval edges (`cr:hasConsent`, `cr:producesSpecimen`, `cr:hasCustodyEvent`, `cr:hasAliquot`, `cr:yieldsResult`, `cr:precededByAct`, each with a natural back-edge) so the whole object is reachable by a *stock broker* `join`, not a materialized inverse view. The **SHACL retrieval view** (`views/blood-draw-admissibility.ttl`) is the attribute projection (the `pick`/`attrs` the screen consumes); `tools/ngsild_view.py` walks it over `examples/blood-draw-context-conformant.ttl` and emits the exact object the screen renders. Guarded by `view 6/6`.

Current suite: SHACL 56/56 · bitemporal 3/3 · projections 18/18 · demo 1/1 · safety 2/2 · pre-IND 2/2 · lims 1/1 · startup 2/2 · schedule 3/3 · usdm 1/1 · ncit 1/1 · view 6/6.
