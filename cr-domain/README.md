...
- **Single-pull retrieval view (NGSI-LD, no recursive lookups)** ✅ the test of the model: an operator pulls **one** entity (the blood-draw collection act) and gets *every* fact behind the green check — participant + current consent, the phlebotomist's venipuncture delegation + credential, the SoA ordering, and specimen → custody → aliquot → result — inlined into **one self-contained object**. The projection is declared as a **SHACL retrieval view** (`views/blood-draw-admissibility.ttl`: forward paths *and* `sh:inversePath` for facts that point back at the hub); `tools/ngsild_view.py` walks it over `examples/blood-draw-context-conformant.ttl` and emits the exact `?join=inline` object the screen consumes. This drove a **modeling correction** — direct retrieval edges (`cr:hasConsent`, `cr:producesSpecimen`, `cr:precededByAct`, `cr:derivedFromCollection`) so a single shape-bounded pull is self-contained instead of forcing the UI into recursive lookups. Guarded by `view 6/6`.
