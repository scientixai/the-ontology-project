...
- **Biospecimen / LIMS lineage** ✅ bitemporal custody chain (state machine → current-state derivation); aliquot/partition lineage (`prov:wasDerivedFrom`); AssayDefinition/LabOrder/AssayResult; blood-draw traceback projection (result → aliquot → primary → draw).
...
- **DTA / DTS vendor data transfer** ✅ an additive, governed extension for Data Transfer Agreements / Specifications (non-CRF vendor data: lab, ECG, imaging, eCOA, DHT). Models the transfer as a **fact graph**, not a document: vendor terms are aliases **resolved once** (`cr:VendorAlias`, compounding corpus) instead of stored per-DTA rows; content binds to a canonical `cr:BiomedicalConcept` and SDTM is a **projection** (`projections/dta_sdtm_lb.rq`); the "Data Validation & Quality" layer the DTA MVP deferred is **executable SHACL** graded Violation/Warning/Info (`shapes/dta-*.ttl`); signatures are credential-backed `top:Attestation`s, blinding a `cr:BlindingConstraint`, amendment a bitemporal `cr:TransferAmendment` whose propagation the shapes force to be explicit. Sodium worked example end-to-end (`examples/dta-lab-safety/`) + deliberately-broken counterpart + vendor connector catalog (Castor/Veeva/Medidata/Oracle, grounded in verified public API docs). Answers to the six open modeling questions and the vendor research in `docs/dta-design-notes.md`.
- **Recruitment & screening funnel** ✅ campaign → candidate (site-boundary, PII-side `cr:Recruit`) → pre-screening → screening with coded outcome (screen failure is a *value*, ADR-0024) → conversion gated on 'eligible' (Violation otherwise); funnel source-attribution Warning; single-pull funnel view.
- **Randomization & blinding (IxRS/RTSM)** ✅ `cr:RandomizationEvent` (subject→arm, stratum, number) + blinded `cr:KitAssignment` + audited `cr:UnblindingEvent` — the blind is never silently broken (reason + authorizer required, Violation); single-pull kit view.
- **Drug supply & IMP accountability** ✅ shipment/dispense/return/destroy as bitemporal events on `hcls:Lot`; the drug log is a **projection** (`accountability_log.rq`); dispensing from a quarantined excursion lot = Violation; unauthorized destruction = Violation.
- **Medical coding (MedDRA/WHODrug)** ✅ `cr:CodeAssignment` as a promoted judgment (verbatim → BYOL dictionary IRI, method, version required) + `cr:CodingReview`; unreviewed autocode = Warning; WHODrug joins MedDRA as BYOL references.
- **ADaM depth** ✅ structure classes (ADSL/BDS/OCCDS) as controlled values, `cr:AnalysisVariable` (PARAMCD), structure-enum shape.
- **Integrated summaries & disclosure** ✅ `cr:IntegratedSummary` (ISS/ISE as *kind* value; ≥1 study by IRI; derives-from required = reproducible-to-source across studies); `cr:RegistryRecord` (NCT; posted results must cite `cr:EndpointResult` by IRI = Violation otherwise); `cr:Publication` traceability Warning.
- **Site closeout** ✅ Close-Out Visit + `cr:Reconciliation` (one class, four scopes as values) + ISF + archival (retention clock Warning) + `cr:SiteClosure` resting on COV + balanced reconciliations (Violations otherwise) — closed by evidence, not declaration.
- **Universal DNA alignment** ✅ aligned to canonical TOP Core: one `top:CommonEntity` root above the 8 CLOs (re-exported locally by `ontology/top-core.ttl`); every entity carries the Universal DNA — `top:identifier` (identity) + `top:observedAt` (time, canonical NGSI-LD term) + `top:status` (lifecycle); `cr:UniversalDNAShape` (the domain application of core's `top:UniversalDNAShape`) enforces it (conformant + negative test).
...
Current suite: coherence 3/3 · seam 2/2 · SHACL 94/94 · bitemporal 3/3 · projections 24/24 · demo 1/1 · safety 2/2 · pre-IND 2/2 · lims 1/1 · startup 2/2 · schedule 3/3 · usdm 1/1 · ncit 1/1 · view 23/23 · competency 8/8 · vocab 4/4 · owl-lint 2/2.
...
> TOP Core (`github.com/scientixai/the-ontology-project`): one root (`top:CommonEntity`) + 8 CLOs +
> Universal DNA (identifier + observedAt + status) + the cr-conventions bitemporal envelope
> (recordedAt/supersededAt/validTo — documented divergences pending upstream alignment).
> Not the authoritative source; replaced by a pinned Core import at extraction (ADR-0023).
