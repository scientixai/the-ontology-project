...
FLOWS = [
    ...
    dict(id="tmf-onboarding", title="TMF onboarding &amp; completeness",
         blurb="Onboard a customer&#8217;s filing cabinet: each document classified to its artifact type and given the bitemporal + provenance envelope, then judged against an essential-documents checklist &mdash; so &#8220;is my TMF inspection-ready?&#8221; becomes a query and a shape, not a manual audit.",
         onto=["tmf-onboarding.ttl"],
         shapes=["tmf:TMFEssentialCompletenessShape", "tmf:TMFRecommendedCompletenessShape",
                 "tmf:FiledArtifactClassifiedShape"],
         proj=["tmf_completeness.rq"], screens=["TMF dashboard"],
         note="Builds on the TMF binding map: a filed document inherits its artifact type&#8217;s content binding, so completeness and comprehension share one model. The expected set is reusable data &mdash; the DIA/ICH essential-documents checklist as a <code>tmf:CompletenessProfile</code>; a <i>current</i> filing satisfies a requirement, a superseded or expired one does not. The single pull returns the expected set <i>and</i> the filed set, so the dashboard diffs them at the glass. Classifying real PDFs and extracting their facts stays the consuming system&#8217;s job."),
