...
def shell(title, active, body):
    return ("<!doctype html><html lang='en'><head><meta charset='utf-8'>"
            ...
            f"<title>{title}</title><style>{CSS}{SCREEN_CSS}</style></head><body><div class='wrap'>"
            f"<nav><div class='brandmark'>{LOGO}<div class='brandsub'>Clinical Research reference graph</div></div>{nav(active)}</nav>"
            f"<main>{body}"
            "<footer>Auto-generated by <code>cr-domain/docs/build_docs.py</code> from the ontology, shapes, "
