...
FLOWS = [
    ...
    dict(id="recruitment", title="Recruitment &amp; screening",
         blurb="The funnel from outreach to enrolled participant: campaign &rarr; candidate (a site-boundary, PII-side identity) &rarr; pre-screening &rarr; consented screening with a coded outcome &rarr; conversion through the attested Enrollment bridge. Screen failure is an outcome value, never a class.",
         onto=["cr-core-recruitment.ttl"],
         shapes=["cr:ScreeningRecordShape", "cr:RecruitConversionShape", "cr:RecruitSourceShape"],
         proj=[], screens=["Recruitment funnel"],
         claim="A screen-failed candidate <b>cannot</b> quietly become a participant: conversion without an &lsquo;eligible&rsquo; screening outcome is caught &mdash; and every candidate carries the source that recruited them, so funnel attribution and diversity reporting have no holes.",
         stops=[
             dict(name="Campaign", time="2026-01", who="sponsor / site", state="done", checks=[("ok", "channel recorded (referral) &mdash; the funnel's source attribution")]),
             dict(name="Candidate", time="2026-02-01", who="site", state="done", checks=[("ok", "PII-side identity &mdash; stays inside the site boundary")]),
             dict(name="Pre-screening", time="2026-02-01", who="coordinator", state="done", checks=[("ok", "proceed &mdash; on to full screening")]),
             dict(name="Screening", time="2026-02-04", who="site", state="done", checks=[("ok", "eligible &mdash; the gate that authorizes conversion"), ("warn", "a screen-failure must cite the failed criterion by IRI")]),
             dict(name="Conversion", time="2026-02-05", who="enroller", state="done", checks=[("ok", "attested Enrollment &rarr; pseudonymous Participant S-014")]),
         ],
         note="PII discipline: the candidate (<code>cr:Recruit</code>) is a site-boundary identity like <code>hcls:Person</code>; downstream clinical data never references it. The only path across the boundary is the attested <code>cr:Enrollment</code> &mdash; the same bridge, entered from the funnel side."),
    ...
    dict(id="randomization", title="Randomization &amp; blinding",
         blurb="The IxRS/RTSM events as first-class bitemporal facts: the randomization act (subject &rarr; arm, under a stratum, with a number), the blinded kit that operationalizes it, and the audited unblinding event &mdash; the blind is never silently broken.",
         onto=["cr-core-randomization.ttl"],
         shapes=["cr:RandomizationEventShape", "cr:KitAssignmentShape", "cr:UnblindingEventShape"],
         proj=[], screens=["Randomization"],
         claim="The blind is <b>never silently broken</b>: an unblinding without a recorded reason and authorizer is caught &mdash; the same audited-event discipline as the database unlock. The site sees the kit; the arm stays with the system.",
         stops=[
             dict(name="Randomization", time="2026-03-02", who="IxRS", state="done", checks=[("ok", "subject &rarr; arm, stratum + number recorded")]),
             dict(name="Kit assignment", time="2026-03-02", who="IxRS", state="done", checks=[("ok", "blinded kit K-1187 &mdash; what the site sees")]),
             dict(name="Dispensing", time="2026-03-02", who="pharmacist", state="done", checks=[("ok", "fulfils the kit &mdash; see IMP accountability")]),
             dict(name="Emergency unblinding", time="2026-05-18", who="medical monitor", state="warn", checks=[("ok", "reason + authorizer recorded &mdash; audited, not silent"), ("warn", "an unblinding missing either is a Violation")]),
         ],
         note="&lsquo;Is randomized&rsquo; is never a flag (first-principles P6): it is the <code>cr:RandomizationEvent</code> with its time, its stratum, and its system of record. The unblinding kind (emergency / planned / regulatory) is a value, not a class (ADR-0024)."),
    dict(id="supply", title="Drug supply &amp; IMP accountability",
         blurb="The investigational-product custody chain: shipment to site, dispensing to a participant under the blinded kit, return, and authorized destruction &mdash; with the drug log as a projection of these events, and a temperature excursion quarantining what it touched.",
         onto=["cr-core-supply.ttl"],
         shapes=["cr:DispensingEventShape", "cr:DestructionEventShape", "cr:TemperatureExcursionShape"],
         proj=["accountability_log.rq"], screens=["IMP accountability"],
         claim="Product never silently leaves the chain: dispensing from a <b>quarantined</b> lot is caught, destruction without an authorizer is caught, and the accountability log is a <b>view</b> of the custody events &mdash; reconciliation is arithmetic, not archaeology.",
         stops=[
             dict(name="Shipment", time="2026-02-27", who="depot", state="done", checks=[("ok", "lot L-2209 dispatched; receipt acknowledged at site")]),
             dict(name="Excursion in transit", time="2026-02-28", who="cold chain", state="warn", checks=[("warn", "9.4&deg;C for 2h &mdash; lot quarantined"), ("ok", "dispositioned RELEASED after review, before any dispensing")]),
             dict(name="Dispensing", time="2026-03-02", who="pharmacist", state="done", checks=[("ok", "30 units to S-042, from the lot, under the kit")]),
             dict(name="Return", time="2026-03-30", who="site", state="done", checks=[("ok", "4 unused units back into custody")]),
             dict(name="Destruction", time="2026-06-15", who="pharmacist", state="done", checks=[("ok", "authorized &mdash; the chain closes")]),
         ],
         note="Reuses the LIMS custody discipline for product instead of specimens: events on an <code>hcls:Lot</code>, chained in valid time. The drug accountability log (<code>projections/accountability_log.rq</code>) renders from the events on request &mdash; the DoA-log pattern, applied to supply."),
    ...
    dict(id="dta", title="External lab data transfer (DTA / DTS)",
         blurb="Vendor non-CRF data (safety labs, ECG, imaging, eCOA) arrives under a Data Transfer Agreement / Specification &mdash; modeled as a fact graph so vendor terms resolve once, SDTM is a projection, and conformance runs at ingestion.",
         onto=["dta-module.ttl"],
         shapes=["cr:DataElementConceptBoundShape", "cr:VendorAliasResolvedShape",
                 "cr:ResolutionProvenanceShape", "cr:TransferProfileMandatoryShape",
                 "cr:DataTransferAgreementShape", "cr:TransferAmendmentPropagationShape"],
         proj=["dta_sdtm_lb.rq", "dta_document.rq"], screens=[]),
    ...
    dict(id="eop2", title="EOP2 &amp; analysis",
         blurb="The back bracket: the analysis traceability spine &mdash; native facts &rarr; ADaM datasets &rarr; Tables/Listings/Figures &rarr; endpoint results &mdash; and the End-of-Phase-2 <b>decision node</b> that rests on it: a coded go / no-go verdict, evidenced by the outputs, that yields a Phase 3 design. Every table is reproducible-to-source and grounded in the SAP; every result traces to its estimand.",
         onto=["cr-core-eop2.ttl", "cr-core-adam.ttl", "cr-core-analysis.ttl"],
         shapes=["cr:EndpointResultTraceShape", "cr:ADaMTraceabilityShape",
                 "cr:TLFTraceabilityShape", "cr:AnalysisDecisionShape",
                 "cr:EOP2ReadinessShape", "cr:GoDecisionDesignShape",
                 "cr:AnalysisDatasetStructureShape"],
         proj=["adam_traceability.rq"], screens=["EOP2 gate"],
         claim="The go / no-go isn&rsquo;t a free-text meeting note &mdash; it&rsquo;s a <b>decision node</b>: a coded verdict that <b>rests on</b> the analysis outputs and yields a Phase 3 design. A pivotal table with no SAP grounding is <b>caught</b> as post-hoc; a decision resting on nothing is <b>caught</b> as decision-by-assertion.",
         stops=[
             dict(name="Data cut", time="2026-08-01", who="sponsor", state="done", checks=[("ok", "locked snapshot for the analysis")]),
             dict(name="ADaM datasets", time="2026-08-01", who="biostatistics", state="done", checks=[("ok", "each derives from native source &mdash; reproducible to the leaf fact")]),
             dict(name="TLF pack", time="2026-08-03", who="biostatistics", state="done", checks=[("ok", "every table traces to an ADaM dataset <b>and</b> the SAP that pre-specified it"), ("warn", "a post-hoc table presented as pivotal is caught")]),
             dict(name="Endpoint result", time="2026-08-02", who="biostatistics", state="done", checks=[("ok", "ORR traces to its endpoint, estimand + SAP (overstatement guard)")]),
             dict(name="EOP2 decision", time="2026-09-15", who="sponsor + FDA", state="done", checks=[("ok", "coded verdict &mdash; GO &mdash; resting on the TLF + result evidence, not a meeting note")]),
             dict(name="Phase 3 design", time="2026-09-15", who="sponsor", state="done", checks=[("ok", "the go-decision yields it &mdash; dose-optimized (Optimus)")]),
         ],
         note="The EOP2 meeting stays a <code>cr:RegulatoryInteraction</code> (with its readiness warning), but the <i>decision</i> is now a first-class <code>cr:AnalysisDecision</code>: a coded outcome that <code>cr:restsOn</code> the analysis outputs and <code>cr:yieldsDesign</code> a Phase 3 design &mdash; so &lsquo;what did the go-decision rest on&rsquo; is a graph walk. A <code>cr:TableListingFigure</code> must be reproducible-to-source (<code>prov:wasDerivedFrom</code> an ADaM dataset) and <code>cr:prespecifiedIn</code> the SAP &mdash; the analysis analog of the overstatement guard."),
    dict(id="csr", title="Reporting &mdash; the CSR",
         blurb="The Clinical Study Report (ICH E3) as a fact graph, not an opaque file: a results section doesn&rsquo;t just assert a conclusion, it <b>presents</b> the pre-specified table and <b>reports</b> the endpoint result it draws from &mdash; each reproducible to its ADaM source. &lsquo;On what does this conclusion rest?&rsquo; is a graph walk from the section to the leaf fact.",
         ...
         stops=[
             ...
             dict(name="Medical writing", time="2026-08-20", who="medical writer", state="done", checks=[("ok", "each results section drafted against its TLF")]),
             dict(name="Results sections trace", time="2026-08-25", who="system", state="done", checks=[("ok", "§11.4 presents Table 14.3.1 <b>and</b> reports the ORR result"), ("warn", "a results section that traces to nothing is caught")]),
             dict(name="Synopsis &amp; narrative", time="2026-08-28", who="medical writer", state="done", checks=[("ok", "narrative sections exempt &mdash; nothing to trace")]),
             dict(name="CSR finalized", time="2026-09-01", who="sponsor", state="done", checks=[("ok", "integrated ICH E3 report &mdash; the narrative the submission carries")]),
         ],
         note="The CSR reuses the analysis spine: <code>cr:CSRSection cr:presentsTLF</code> a <code>cr:TableListingFigure</code> and <code>cr:reportsResult</code> an <code>cr:EndpointResult</code>. Only <code>efficacy-results</code> / <code>safety-results</code> sections must trace; synopsis / narrative / administrative sections are exempt by <code>cr:sectionKind</code>."),
    dict(id="submission", title="Submission &mdash; eCTD",
         blurb="The terminal node: the eCTD submission as a <b>manifest</b>, not a new source of truth. It references the hard-locked database (A1), the Clinical Study Report (A3), the ADaM datasets (A2), and the Define-XML &mdash; every arrow points back at already-produced, locked content. This is where TOP-CR ends.",
         onto=["cr-core-submission.ttl", "cr-core-disclosure.ttl"],
         shapes=["cr:eCTDSubmissionShape", "cr:SubmissionLockHardShape",
                 "cr:SubmissionCompletenessShape", "cr:IntegratedSummaryShape",
                 "cr:RegistryRecordShape", "cr:PublicationTraceShape"],
         ...
         stops=[
             ...
             dict(name="CSR + datasets + Define", time="2026-09", who="sponsor", state="done", checks=[("ok", "the report, the ADaM datasets, and the map to read them")]),
             dict(name="eCTD assembled", time="2026-10-01", who="regulatory affairs", state="done", checks=[("ok", "Module 5 manifest references locked content &mdash; nothing re-authored")]),
             dict(name="Submitted to regulator", time="2026-10-01", who="sponsor", state="done", checks=[("ok", "NDA / BLA &mdash; the terminal projection of the trial")]),
             dict(name="Beyond eCTD", time="&mdash;", who="&mdash;", state="pend", checks=[("pend", "label / REMS / HTA are sibling domains &mdash; deliberately out of scope")]),
         ],
         note="Scope boundary, made explicit: TOP-CR runs trial &rarr; regulatory submission (eCTD Module 5). Product-lifecycle (label, REMS, PSUR) and commercialization (HTA, market access) are sibling domains, out of scope; the unit of analysis stays the study. <code>cr:builtOnLock</code> is the seam that ties B1 back to A1 &mdash; the submission must rest on a HARD <code>cr:DatabaseLock</code>."),
    dict(id="coding", title="Medical coding",
         blurb="The coding workflow the term crosswalks alone don&rsquo;t carry: a code assignment is a promoted judgment &mdash; &lsquo;this verbatim means this MedDRA/WHODrug code&rsquo; &mdash; with its method (autocoder vs manual), its dictionary version, its coder, and its human review.",
         onto=["cr-core-coding.ttl"],
         shapes=["cr:CodeAssignmentShape", "cr:AutocodedReviewShape"],
         proj=[], screens=["Medical coding"],
         claim="An autocoded term is a <b>suggestion, not a judgment</b>, until a human coder reviews it &mdash; an unreviewed autocode is flagged; an assignment without its dictionary version (meaningless at reconciliation and at submission) is caught.",
         stops=[
             dict(name="Verbatim captured", time="2026-03-10", who="site", state="done", checks=[("ok", "&lsquo;achy joints&rsquo; &mdash; the investigator's words, preserved")]),
             dict(name="Autocoder pass", time="2026-03-11", who="system", state="warn", checks=[("ok", "&rarr; MedDRA PT Arthralgia (27.0), by IRI"), ("warn", "unreviewed at this point &mdash; a suggestion")]),
             dict(name="Human review", time="2026-03-12", who="medical coder", state="done", checks=[("ok", "approved &mdash; now a judgment with authority")]),
             dict(name="ConMed coding", time="2026-03-11", who="medical coder", state="done", checks=[("ok", "&lsquo;baby aspirin&rsquo; &rarr; WHODrug (manual), version carried")]),
         ],
         note="MedDRA and WHODrug are licensed terminologies referenced by IRI only (bring-your-own-license) &mdash; the model carries the coding <i>workflow</i> and never reproduces dictionary content. The assignment has the same promoted-fact shape as <code>cr:CausalityAssessment</code> (ADR-0024)."),
    dict(id="site-closeout", title="Site closeout",
         blurb="The site-side of closeout, counterpart of the database lock: the Close-Out Visit, the reconciliations (IMP, device, documents, queries &mdash; one act, four scopes), the archived Investigator Site File with its retention clock, and a closure that rests on all of it.",
         onto=["cr-core-site-closeout.ttl"],
         shapes=["cr:ReconciliationShape", "cr:SiteClosureShape", "cr:ArchivalShape"],
         proj=[], screens=["Site closeout"],
         claim="A site is closed by <b>evidence, not declaration</b>: a closure without a Close-Out Visit is caught, and a closure resting on a <b>discrepant</b> reconciliation is caught &mdash; 12 unaccounted units block the door on the way out.",
         stops=[
             dict(name="Close-Out Visit", time="2026-08-10", who="CRA", state="done", checks=[("ok", "the closing bracket of the SIV")]),
             dict(name="IMP reconciliation", time="2026-08-10", who="CRA + pharmacist", state="done", checks=[("ok", "balanced &mdash; against the accountability-log projection")]),
             dict(name="Document reconciliation", time="2026-08-10", who="CRA + coordinator", state="done", checks=[("ok", "ISF complete; essential records accounted for")]),
             dict(name="Archival", time="2026-08-15", who="site", state="done", checks=[("ok", "ISF into retention &mdash; clock runs to 2051")]),
             dict(name="Site closure", time="2026-08-20", who="sponsor", state="done", checks=[("ok", "rests on the COV + balanced reconciliations")]),
         ],
         note="One <code>cr:Reconciliation</code> class, four scopes (IMP / device / documents / queries) &mdash; a classification carried as a value, not four classes (ADR-0024). The IMP scope reconciles against the supply module's event chain: arithmetic over a projection, never a spreadsheet."),
...
def nav(active):
    NAV = [
        ...
        ("Conduct", [
            ("recruitment.html",    "Recruitment &amp; screening", "recruitment"),
            ("enrollment.html",     "Enrollment &amp; consent", "enrollment"),
            ("randomization.html",  "Randomization &amp; blinding", "randomization"),
            ("supply.html",         "Drug supply (IMP)",     "supply"),
            ("schedule.html",       "Schedule of Activities", "schedule"),
            ("edc.html",            "Data capture (EDC)",    "edc"),
            ("lims.html",           "Biospecimen &amp; LIMS","lims"),
            ("dta.html",            "External lab data (DTA)", "dta"),
            ("safety.html",         "Safety &amp; PV",       "safety"),
            ("rbqm.html",           "Risk-based monitoring", "rbqm"),
            ("deviation.html",      "Deviations &amp; CAPA", "deviation"),
        ]),
        ("Closeout &amp; submission", [
            ("site-closeout.html",  "Site closeout",         "site-closeout"),
            ("coding.html",         "Medical coding",        "coding"),
            ("dblock.html",         "Database lock &amp; follow-up", "dblock"),
            ("eop2.html",           "Analysis &amp; EOP2 decision", "eop2"),
            ("csr.html",            "Reporting (CSR)",       "csr"),
            ("submission.html",     "Submission (eCTD)",     "submission"),
        ]),
        ("Governance", [
...
# ── Unified rich style + interactivity, applied to EVERY page ─────────────────
# The whole site is built one way: the rich chrome (RICH_CSS = the superset of
# component styles used by the operator pages) + Tailwind (for the tab utility
# classes) + the shared JS (tab toggle, reconcile / projection modals — all
# guarded, inert on pages that don't use them). No page is special-cased.
_SC = os.path.join(ROOT, "docs", "scenarios")
RICH_CSS = open(os.path.join(_SC, "_rich.css")).read() if os.path.exists(os.path.join(_SC, "_rich.css")) else ""
UNIFIED_JS = open(os.path.join(_SC, "_rich.js")).read() if os.path.exists(os.path.join(_SC, "_rich.js")) else ""
TAILWIND = '<script src="https://cdn.tailwindcss.com"></script>'


def scenario(page_id):
    """A page may supply a hand-authored rich body (hero / screen / tabs / modals)
    at docs/scenarios/{id}.html. If present, the generator slots it in verbatim;
    otherwise the page is generated. Same build path for every page."""
    p = os.path.join(_SC, f"{page_id}.html")
...
def shell(title, active, body):
    return ("<!doctype html><html lang='en'><head><meta charset='utf-8'>"
            ...
            f"<title>[DRAFT] {title}</title>{TAILWIND}"
            f"<style>{CSS}{SCREEN_CSS}{RICH_CSS}{DRAFT_CSS}</style></head><body>"
            ...
            f"<main>{body}"
            "<footer><b>Draft / pre-release</b> &mdash; not finalized. Built by "
            "<code>cr-domain/docs/build_docs.py</code> &mdash; the rich shell + model "
            "tables for every page, with hand-authored scenarios slotted in where present."
            f"</footer></main></div>{UNIFIED_JS}</body></html>")
...
&lsquo;reference graph, not a runtime&rsquo; note on the <a href="index.html">Overview</a>.</p></div></div>"""
    return (
        ...
        "<h2>How to read a .ttl file &mdash; it's sentences</h2>"
        "<p class='lead'>Every file in this reference is a set of <b>subject &ndash; predicate &ndash; object</b> "
        "sentences. That's not a style we chose; it's what RDF <i>is</i>. Turtle's semicolons just keep the "
        "same subject going. Open any worked example and read it aloud:</p>"
        "<pre>ex:bd-coll a cr:StudyEvent ;              <span class='c'># The blood-draw collection is a study event;</span>\n"
        "    cr:hasConsent      ex:bd-consent ;    <span class='c'># &hellip;it has consent bd-consent;</span>\n"
        "    cr:precededByAct   ex:bd-ecg-act ;    <span class='c'># &hellip;it was preceded by the ECG;</span>\n"
        "    cr:producesSpecimen ex:bd-spec ;      <span class='c'># &hellip;it produces the whole-blood specimen;</span>\n"
        "    cr:yieldsResult    ex:bd-result .     <span class='c'># &hellip;and it yields the assay result.</span>\n"
        "\n"
        "ex:bd-spec cr:hasCustodyEvent ex:bd-ce-recv .  <span class='c'># The specimen has custody event 'received'.</span>\n"
        "ex:bd-ce-recv cr:fromState \"SampleDue\" ;      <span class='c'># That event went from state SampleDue&hellip;</span>\n"
        "    cr:toState \"Received\" .                   <span class='c'># &hellip;to state Received.</span></pre>"
        "<p>Thirteen sentences about one blood draw &mdash; that's the whole trick. Property names are the "
        "<b>verb phrase</b> of each sentence (<code>hasConsent</code>, <code>producesSpecimen</code>), which is why "
        "they read naturally in a triple even though a name like <code>hasConsent</code> might look like a yes/no "
        "flag in isolation. It never is: booleans are banned outright in this model (first-principles P6), so the "
        "value of any <code>has&hellip;</code> sentence is always another entity you can walk to.</p>"
        "<p class='note'><b>Statements about statements.</b> Sometimes the important fact is about another fact "
        "&mdash; <i>who</i> asserted that this consent covers that draw, with what confidence, superseded when? "
        "RDF's built-in answer (reification) is clunky, and this reference deliberately never uses it. Instead the "
        "claim itself is <b>promoted to an entity</b> with its own sentences: the <code>cr:Attestation</code>, the "
        "<code>cr:CodeAssignment</code> (verbatim &rarr; code, with its coder and review), the <code>cx:Mapping</code> "
        "(with its confidence and curator), the bitemporal supersession chain. If you find yourself wanting to "
        "annotate a triple, you almost certainly want a promoted fact &mdash; that's Principle 6, and it's why an "
        "auditor's question is always a graph walk, never an excavation.</p>"
...
def build():
    """One build path for every page: the rich shell + model-driven tables, with a
    hand-authored scenario (docs/scenarios/{id}.html) slotted in verbatim where one
    exists. No page is special-cased and nothing is skipped — a scenario is just a
    richer body for a page that has one; every page is (re)built the same way."""
    ...
    scen = sorted(f[:-5] for f in os.listdir(_SC) if f.endswith(".html")) if os.path.isdir(_SC) else []
    print(f"Built {len(pages)} pages to {out}/ (one path, rich shell for all)")
    print(f"  scenario overrides slotted in: {', '.join(scen) if scen else '(none)'}")
