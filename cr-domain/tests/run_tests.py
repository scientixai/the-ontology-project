...
def entity_view_checks(failures):
    """The reference entity-view corpus (views/entity/*.ttl, generated by
    tools/gen_entity_views.py from the OOUX catalog) is well-formed:
      (1) every file parses; the corpus covers the whole catalog (>= 70 views);
      (2) the entity-view vocabulary (cr:EntityView + cr:isIdentity) is declared;
      (3) every cr:EntityView has a sh:targetClass, >= 1 sh:property, and a
          NON-EMPTY identity set (>= 1 cr:isIdentity property) — the depth-0
          render of a pointer needs it;
      (4) a property is either an attribute (sh:datatype) or a pointer (sh:node),
          never both, and every pointer resolves to a cr:...EntityView target
          (so the corpus is internally navigable). This structurally guards the
          generic viewer's contract without validating the flat menu as a
          constraint (it is a reference recipe, not a runtime shape)."""
    from rdflib import Namespace, OWL, RDFS
    ev_dir = os.path.join(ROOT, "views", "entity")
    if not os.path.isdir(ev_dir):
        return 0, 0
    files = sorted(glob.glob(os.path.join(ev_dir, "*.ttl")))
    SHN = Namespace("http://www.w3.org/ns/shacl#")
    EV = URIRef(str(CR) + "EntityView")
    ISID = URIRef(str(CR) + "isIdentity")
    total = passed = 0

    # (1) every file parses; merge into one graph
    total += 1
    merged = Graph()
    parse_fail = []
    for f in files:
        try:
            ...
            parse_fail.append(f"{os.path.basename(f)}: {e}")
    if not parse_fail:
        passed += 1; print(f"[PASS] entity-view: all {len(files)} corpus files parse")
    else:
        for pf in parse_fail:
            failures.append(("ENTITYVIEW", "parse", pf))
        print(f"[FAIL] entity-view: {len(parse_fail)} file(s) failed to parse")

    views = set(merged.subjects(RDF.type, EV))

    # (2) vocabulary declared; corpus covers the catalog
    total += 1
    ...
    bad = []
    for v in views:
        props = list(merged.objects(v, SHN.property))
        has_target = merged.value(v, SHN.targetClass) is not None
        has_ident = any((p, ISID, None) in merged for p in props)
        if not has_target or not props or not has_ident:
            bad.append(f"{_ln_local(v)}(target={has_target},props={len(props)},identity={has_ident})")
    if not bad:
        passed += 1
        print(f"[PASS] entity-view: every view has a targetClass, properties, and a non-empty identity set")
    else:
        failures.append(("ENTITYVIEW", "shape", "; ".join(sorted(bad)[:10])))
        print(f"[FAIL] entity-view: {len(bad)} malformed view(s)")

    # (4) property is attribute XOR pointer; pointers resolve to EntityViews
    total += 1
    problems = []
    for v in views:
        for p in merged.objects(v, SHN.property):
            dt = merged.value(p, SHN.datatype)
            node = merged.value(p, SHN["node"])
            if dt is not None and node is not None:
                problems.append(f"{_ln_local(v)}: property both datatype+node")
            if node is not None and not str(node).endswith("EntityView"):
                problems.append(f"{_ln_local(v)}: pointer to non-view {node}")
    if not problems:
        passed += 1
        print(f"[PASS] entity-view: attribute/pointer split clean; all pointers target entity views")
    else:
        failures.append(("ENTITYVIEW", "pointer", "; ".join(sorted(set(problems))[:10])))
        print(f"[FAIL] entity-view: {len(set(problems))} property problem(s)")
